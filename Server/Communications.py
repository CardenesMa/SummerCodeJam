...
class ServerComms:
	...
	def handle_client_action(self,  user:uuid.UUID | str, action:str, action_object):
		returns = []
		if action in self.action_callbacks:
...
class ServerManager:
	...
	by an action sent through the packet. 
	"""
	
	def __init__(self, server_comms = ServerComms()):
		self.server_comms = server_comms
		# put all the actions and callbacks together into a dict (server_comms.action_callbacks)
